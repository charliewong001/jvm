...
/**
 * VM Args
 * -Xss2M 虚拟机栈大小(注意，设置的是每一个虚拟机栈的大小，而不是虚拟机栈的总的大小。
 * -Xss2M 设置的意思就是,每创建一个线程，就创建一个虚拟机栈，那么这个虚拟机栈就是2M)
 * @author Charlie
 * @description 测试虚拟机栈(VMStack)OutOfMemeryError
 * 一个线程有一个虚拟机栈，虚拟机栈的生命周期与线程相同。
 * 设置虚拟机栈总的大小之后，不停地创建线程，就会导致虚拟机栈内存溢出
 * 
 ...
 * 但是这样产生的内存溢出异常与栈空间是否足够大并不存在任何联系，
    ...
    剩余的内存为2GB（操作系统限制）减去Xmx（最大堆容量），再减去MaxPermSize（最大方法区容量）。
    程序计数器消耗内存很小，可以忽略掉。 如果虚拟机进程本身耗费的内存不计算在内,
    剩下的内存就由虚拟机栈和本地方法栈“瓜分”了。 每个线程分配到的栈容量越大，可以
